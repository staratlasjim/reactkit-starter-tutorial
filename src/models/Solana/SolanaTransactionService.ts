...
export class SolanaTransactionService {
  ...
  public async sendTransactionsWithManualRetry(
    ...
    let toRemoveSigners = new Array<boolean>();
  ...
  public async sendTransactions(
    ...
    if (!this.walletModel.pubKey) throw new Error('WalletNotConnectedError');
    ...
    for (let i = 0; i < instructionSet.length; i++) {
      ...
      let transaction = new Transaction({ feePayer: walletAddress });
  ...
  public async sendTransaction(
    ...
    if (!this.walletModel.publicKey) throw new WalletNotConnectedError();
  ...
  async simulateTransaction(transaction: Transaction, commitment: Commitment): Promise<any> {
    // @ts-ignore
    transaction.recentBlockhash = await this.solanaModel.connection._recentBlockhash(
      // @ts-ignore
